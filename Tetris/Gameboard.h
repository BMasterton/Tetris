...
//      easier for us to think of this grid in terms of XY coordinates. Heres the 
...
public:
	...
	int getContent(const int columnX, const int rowY) const;
	...
	void setContent(const Point& point, const int contentToSet);
	...
	void setContent(const int columnX, const int rowY, const int contentToSet);
	...
	void setContent(std::vector<Point>& locationPoints, int contentToSet);
	...
	bool areAllLocsEmpty(const std::vector<Point> pointTestLocations) const;
...
private:  // This is commented out to allow us to test. 
	...
	bool isRowCompleted(const int rowIndexToTest) const;
	...
	// - return: nothing
	void copyRowIntoRow(const int sourceRowIndex, const int targetRowIndex);
	...
	// - return: nothing
	void removeRows(const std::vector<int>& rowIndicesToRemove);
