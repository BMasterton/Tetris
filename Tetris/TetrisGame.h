...
public:
	...
	static constexpr double MIN_SECONDS_PER_TICK = 0.20; // the fastest "tick" rate (in seconds), init to 0.20
	static const int TOTAL_BLOCK_IMAGES = 7;
	static const int MAX_X_WIDTH = 9;
	static const int MAX_Y_HEIGHT = 19;
private:
	// MEMBER VARIABLES
	...
	const Point gameboardOffset = Point(64,128);	// pixel XY offset of the gameboard on the screen
	const Point nextShapeOffset = Point(128, 672);	// pixel XY offset to the nextShape
...
private:
	...
	// return: nothing
	void drawBlock(const Point& topLeft, const int xOffset, const int yOffset, const TetColor& color);
